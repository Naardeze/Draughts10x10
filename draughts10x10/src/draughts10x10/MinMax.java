...
final class MinMax extends HashMap<String, Integer> {//<position, move value>    
    ...
    final private static int MAX = Integer.MAX_VALUE;//max
    final private static int MIN = Integer.MIN_VALUE;//min
    ...
    static ArrayList<Integer> getAIMove(int ai, char[] position, HashSet<Integer>[] pieces, HashMap<Integer, Move[]> moves) {
        ...
        for (int from : moves.keySet()) {
            ...
            for (Move move : moves.get(from)) {
                ...
                int to = move.getTo();
                //min
                int beta = new MinMax(Node.BETA, player).valueOf(getPosition(ai, position.clone(), piece, captures, to), opponent ^ capture, turn ^ (1l << from ^ 1l << to), new MinMax(Node.ALFA, ai), new int[] {MIN, MAX}, AI.getValue());
