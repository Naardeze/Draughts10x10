...
final class Draughts10x10 extends JFrame {
    ...
    final static JCheckBox HINT = new JCheckBox("Hint");//hint on/off (off by default)
    ...
    private Draughts10x10() {
        ...
        JMenu gameMenu = new JMenu("Game");//WHITE, BLACK
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (char piece : WB.toCharArray()) {
            IMAGE[0][WB.indexOf(piece)] = ImageIO.read(new File(piece + ".png")).getScaledInstance(size, size, Image.SCALE_SMOOTH);//pawn (w, b)
            IMAGE[1][WB.indexOf(piece)] = ImageIO.read(new File(piece + "k.png")).getScaledInstance(size, size, Image.SCALE_SMOOTH);//king (wk, bk)
        }
