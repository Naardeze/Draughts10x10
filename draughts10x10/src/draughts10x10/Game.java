...
final class Game extends JLayeredPane implements ActionListener {
                    ...
                    if (square[index].contains(e.getPoint())) {//pressed square
                            ...
                            if (index != step && Math.abs(x(index) - x(step)) == Math.abs(y(index) - y(step))) {
                                ...
                                if (pieces[1 - player].contains(step) && !captures.contains(step)) {
                                    ...
                                    if (step == index) {
                                        ...
                                        if (captures.indexOf(step) == maxCapture) {//boardMove
                                            new Thread(new BoardMove(selected, captures)).start();
                                        } else {//continue
                        ...
                        //selected, index is empty
                            ...
                            for (ArrayList<Integer> move : moves.get(selected)) {
                                if (move.get(maxCapture) == index) {//boardMove
                        ...
                        if (pieces[1 - color].contains(step) && vertical.canStep(step)) {
                            ...
                            if (position[step] == EMPTY) {
                                ...
                                do {
                                    ...
                                    do {
                                        captures.add(destination.pop());
                                    } while (pieces[1 - color].contains(destination.getFirst()));
    ...
    private class BoardMove extends Component implements Runnable {
        ...
        BoardMove(int index, ArrayList<Integer> move) {//ai
