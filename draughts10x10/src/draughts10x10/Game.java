...
final class Game extends JLayeredPane implements ActionListener {
                        ...
                        if (pieces[1 - color].contains(step) && vertical.canStep(step)) {
                            ...
                            if (position[step] == EMPTY) {
                                ...
                                do {
                                    ...
                                    //fill captures
                                    //move will contain only empty square(s)
                                    do {
            ...
            if (moves.isEmpty()) {//game over
                ...
                new Thread(){
                    ...
                    public void run() {
                        ...
                        new BoardMove(color, MinMax.getAIMove(color, position, pieces, moves)).run();
                    }
                }.start();
            }
    ...
    final private static int FRAMES = 36;//frames p square
    ...
    private class BoardMove extends Component implements Runnable {
        ...
        public void run() {
            ...
            for (int step : positionBoard.getMove()) {//<<captures>, destination>
                ...
                } while (direction.x * (x(step) - x(index)) != -direction.y * (y(step) - y(index)));//x!=-y -> index=step (0=-0), angle (x=-y)
