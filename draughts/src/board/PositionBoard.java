...
public class PositionBoard extends AbstractBoard {
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < position.length; i++) {
            if (position[i] != EMPTY) {
                g.drawImage(IMAGE[(String.valueOf(position[i]).toLowerCase() + String.valueOf(position[i]).toUpperCase()).indexOf(position[i])][WB.indexOf(String.valueOf(position[i]).toLowerCase())], square[i].x, square[i].y, square[i].width, square[i].height, null);
            }
        }
    }
    ...
    static {
        for (char piece : WB.toCharArray()) {
            try {
                ...
                IMAGE[1][WB.indexOf(piece)] = ImageIO.read(new File(String.valueOf(piece) + "k.png"));
            } catch (Exception ex) {}
        }
    }
}
