...
final class Game extends JLayeredPane implements ActionListener {
    ...
    private void turn(int color) {
        ...
        //2 evaluation
        if (TILEBOARD.isAncestorOf(this)) {//continue only if this (game) is on TILEBOARD; prevent enable UNDO by new game during AI search
            if (moves.isEmpty()) {//game over
                ...
                new Thread(){
                    ...
                    public void run() {
                        ...
                        new BoardMove(MinMax.getAIMove(color, board, pieces, moves, maxCapture, AI.getValue())).run();
                    }
                }.start();
            }
    ...
    private class BoardMove extends Component implements Runnable {
        ...
        BoardMove(int color, int index, ArrayList<Integer> move) {
            ...
            point = TILEBOARD.tile[index].getLocation();
        ...
        public void run() {
            ...
            //move
            for (int step : pieceBoard.getMove()) {
                do {
                    index = direction.getStep(index);
                    
                    //point -> tile[to]
                    for (int horizontal = TILEBOARD.tile[index].x - point.x, vertical = TILEBOARD.tile[index].y - point.y, i = FRAMES - 1; i >= 0; i--) {
                        point.setLocation(TILEBOARD.tile[index].x - (int) (i * (float) horizontal / FRAMES), TILEBOARD.tile[index].y - (int) (i * (float) vertical / FRAMES));
            ...
            //promotion
            if (piece == PAWN[color] && ((color == WHITE && index < GRID / 2) || (color == BLACK && index >= TILEBOARD.tile.length - GRID / 2))) {
                piece = KING[color];
            }
