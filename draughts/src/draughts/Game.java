...
final class Game extends JLayeredPane implements ActionListener {
    ...
    private HashMap<Integer, LinkedList<Integer>[]> moves;
                ...
                for (int pressed = 0; pressed < TILEBOARD.tile.length; pressed++) {
                    if (TILEBOARD.tile[pressed].contains(e.getPoint())) {
                        ...
                        if (selected != NONE && !pieceBoard.getMove().isEmpty() && (pieceBoard.getIndex(pressed) == EMPTY || pressed == selected)) {
                            LinkedList<Integer> move = new LinkedList(pieceBoard.getMove());
    ...
    private void turn(int color) {
        ...
        for (int from : pieces[color]) {
                    ...
                    if (vertical.canStep(from)) {
                        ...
                        if(board[step] == EMPTY && (piece == KING[color] || vertical == horizontal[color])) {
                            if (maxCapturePiece == 0) {//legal move
                                movesPiece.add(new LinkedList(Arrays.asList(new Integer[] {step})));//<step>
                            }
                        ...
                        if (pieces[opponent].contains(step) && vertical.canStep(step)) {
                            ...
                            if (board[step] == EMPTY) {
                                ...
                                LinkedList<LinkedList<Integer>> captureMoves = new LinkedList(Arrays.asList(new LinkedList[] {captureMove}));//<capture, step(s)>

                                board[from] = EMPTY;

                                //check for extra captures
                                do {
                                    LinkedList<Integer> destination = captureMoves.remove(0);//captures + empty
                                    LinkedHashSet<Integer> captured = new LinkedHashSet();

                                    //separate captured from empty
                                    do {
                                    ...
                                    for (int to : destination) {//empty tile(s) after captures
                                        if (captured.size() == maxCapturePiece) {//legal move
                                            LinkedList<Integer> move = new LinkedList(captured);
                                        ...
                                        for (Direction diagonal : Direction.values()) {
                                            if (diagonal.canStep(to)) {
                                                ...
                                                if (piece == KING[color] && !destination.contains(step)) {//no dubbel checks
                                                ...
                                                if (pieces[opponent].contains(step) && !captured.contains(step) && diagonal.canStep(step)) {
                                                    ...
                                                    if (board[step] == EMPTY) {
                                                        captureMove = new LinkedList(captured);
                                                        ...
                                                        captureMoves.addLast(captureMove);//<<captured>, capture, step(s)>
                                                    }
                                                }
                                            }
                                        }
            ...
            if (!movesPiece.isEmpty()) {
                ...
                moves.put(from, movesPiece.toArray(new LinkedList[movesPiece.size()]));
            }
        }
            ...
            if (moves.isEmpty()) {//game over
                ...
                new Thread(){
                    ...
                    public void run() {
                        ...
                        new BoardMove(MinMax.getAIMove(color, board, pieces, moves, LEVEL.getValue())).run();
                    }
                }.start();
            }
        }
    ...
    private class BoardMove extends Component implements Runnable {
        ...
        BoardMove(int from, LinkedList<Integer> move) {//player
        ...
        BoardMove(int color, int from, LinkedList<Integer> move) {
            this.color = color; 
        ...
        public void run() {
            ...
            for (int i = 0; i < maxCapture; i++) {
                pieceBoard.setIndex(pieceBoard.getMove().removeFirst(), EMPTY);
                pieceBoard.repaint();
