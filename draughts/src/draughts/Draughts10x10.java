 ...
 * Draughts10x10 (main)
 * 
 * TILEBOARD -> all games are played on.
 * 
 * Game (WHITE/BLACK)
 * LEVEL (1-5)
 * ARROW (undo move)
...
public class Draughts10x10 extends JFrame {
    ...
    final static JSlider LEVEL = new JSlider(1, 5);//AI search depth -> 1 level = 2 moves (alfa beta)
    ...
    private Draughts10x10(int boardSize) {
        ...
        for (int color : new int[] {WHITE, BLACK}) {
            menu.add(new JMenuItem(new ImageIcon(Toolkit.getDefaultToolkit().createImage(PAWN[color] + ".png").getScaledInstance(44, 44, Image.SCALE_SMOOTH)))).addActionListener(e -> {
        ...
        LEVEL.setPaintTicks(true);//no labels -> no extra distraction
        LEVEL.setOpaque(false);
        LEVEL.setToolTipText("" + LEVEL.getValue());//level
