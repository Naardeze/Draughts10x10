...
final class PieceBoard extends JComponent {
    final private static char W = 'w';//'w'hite pawn
    final private static char B = 'b';//'b'lack pawn
    ...
    private char[] board = new char[TILEBOARD.tile.length];//begin position in constructor
    private ArrayList<Integer> move = new ArrayList();
    
    PieceBoard() {//begin position
        Arrays.fill(board, 0, board.length / 2 - GRID / 2, B);//0-19
        Arrays.fill(board, board.length / 2 - GRID / 2, board.length / 2 + GRID / 2, EMPTY);//20-29
        Arrays.fill(board, board.length / 2 + GRID / 2, board.length, W);//30-49
    }
